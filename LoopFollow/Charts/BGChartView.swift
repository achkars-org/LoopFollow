...
private struct MainBGChart: View {
    ...
    /// BG-per-U/hr conversion so basal rate plots on the same y-axis as BG.
    /// Matches the scale BGChartCanvas uses to draw the basal rectangles.
    ...
    private func bandAnchor(at date: Date, value: Double) -> SelectionAnchor? {
        ...
        for step in model.basal where date >= step.start && date <= step.end {
            let stepTop = step.rate * basalScale
            if value >= 0, value <= stepTop {
                return SelectionAnchor(date: date, value: stepTop / 2, texts: [step.pillText])
            }
        }
        return nil
    }
